...
word2vec = np.load('/home/anonymous/Sememe/data/word2vec.npy')
correct1 = np.load('/home/anonymous/headline.t.npy')
correct2 = np.load('/home/anonymous/headline.tsep.npy')
common = (correct1 == correct2).astype(float)
print(common)
word2topk = {}
...
def score(src_word, tgt_word):
    v1 = word2vec[word2id[src_word]]
    v2 = word2vec[word2id[tgt_word]]
...
for pos in ['a', 'n', 'd']:
    if 'train' in fname:
        ...
        mode = 'valid'
    out_fnames[pos] = fname.replace('_%s.tsv' % mode, '_advvec_%s_%s.tsv' % (pos, mode))
print('Input:', fname)
print('Output:')
        ...
        for i, (word, label) in enumerate(zip(text1_split, label1)):
            if label == 'n' or label == 'a' or label == 'd':
                ...
                for similar_word in similar_words:
                    ...
                    # print(text1_split[:i], similar_word, text1_split[i+1:])
