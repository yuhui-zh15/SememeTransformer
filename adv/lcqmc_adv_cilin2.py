...
with open(fname) as fin:
    for i, line in enumerate(tqdm(fin.readlines())):
        ...
        for i, (word, label) in enumerate(zip(text1_split, label1)):
            if word not in text2_split: continue
            if label == 'n' or label == 'a' or label == 'd':
                similar_words = get_top_k_words(word, label, 3)
                for similar_word in similar_words:
                    new_text1 = ' '.join(text1_split).replace(word, similar_word)
                    new_text2 = ' '.join(text2_split).replace(word, similar_word)
                    fouts[label].write((new_text1 + ' _start_ ' + new_text2).replace('#', ' <unk> ').replace('$', ' <N> ').replace('  ', ' ') + '\t' + text_label + '\n')
