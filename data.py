...
def pad_batch(batch, encoder, pad_start_end=False):
...
import random
def get_data(encoder, data_dir, prefix, cut_down_len, label_size, ratio):
    ...
    for data_type in ['train', 'valid', 'test']:
        ...
        with open(data_path, 'r') as f:
            for line in f:
                ...
                if len(columns) > 1:
                    label[data_type].append(int(columns[1]))
                else:
                    label[data_type].append([])
    ...
    if ratio != 1.0:
        print('shuffle and pick data')
        random.seed(333)
        idxs = list(range(len(text['train'])))
        random.shuffle(idxs)
        idxs = idxs[:int(ratio * len(text['train']))]
        print(idxs[:20])
        print(len(idxs))
        text['train'] = text['train'][idxs]
        label['train'] = label['train'][idxs]
